...
/**
 * Vehicle is an abstract class specifying the basic state of a vehicle and 
 * the methods used to set and access that state. A vehicle is created upon 
 * arrival, at which point it must either enter the car park to take a vacant 
 * space or become part of the queue. If the queue is full, then the vehicle 
 * must leave and never enters the car park. The vehicle cannot be both parked
 * and queued at once and both the constructor and the parking and queuing 
 * state transition methods must respect this constraint. 
 * 
 * Vehicles are created in a neutral state. If the vehicle is unable to park 
 * or queue, then no changes are needed if the vehicle leaves the car park 
 * immediately.
 * Vehicles that remain and can't park enter a queued state via 
 * {@link #enterQueuedState() enterQueuedState} and leave the queued state via 
 * {@link #exitQueuedState(int) exitQueuedState}. 
 * Note that an exception is thrown if an attempt is made to join a queue when 
 * the vehicle is already in the queued state, or to leave a queue when it is not. 
 * 
 * Vehicles are parked using the {@link #enterParkedState(int, int) enterParkedState} 
 * method and depart using {@link #exitParkedState(int) exitParkedState}
 * 
 * Note again that exceptions are thrown if the state is inappropriate: 
 * vehicles cannot be parked or exit the car park from a queued state. 
 * 
 * The method javadoc below indicates the constraints on the time and other 
 * parameters. Other time parameters may vary from simulation to simulation 
 * and so are not constrained here.  
 * 
...
public abstract class Vehicle {
	 ...
	 * @param arrivalTime
	 *            int time (minutes) at which the vehicle arrives and is 
	 ...
	 *            parkingTime < 0, or if intendedDuration is less than the minimum 
	...
	public void enterParkedState(int parkingTime, int intendedDuration) 
		...
		if(this.state == State.P || this.state == State.Q){
			...
			throw new VehicleException("intendedDuration must be greater than or "
					+ "equal to the minimum stay");
		}
	 ...
	 * Queuing formally starts on arrival and ceases with a call to 
	 * {@link #exitQueuedState(int) exitQueuedState}
	 * 
	 * @throws VehicleException
	 ...
	 * @throws VehicleException
	 *            if the vehicle is not in a parked state, is in a queued
	...
	public void exitParkedState(int departureTime) throws VehicleException {
		if(this.state != State.P){
			...
			throw new VehicleException("departureTime must be greater than "
					+ "or equal to parkingTime.");
		}
	 ...
	 * Queuing formally starts on arrival with a call to 
	 * {@link #enterQueuedState() enterQueuedState}
	 ...
	 * 
	 * @return true
	 *            if the vehicle is in a parked state; false otherwise
	 */
	 ...
	 * 
	 * @return true
	 *            if vehicle is in a queued state, false otherwise 
	 */
	 ...
	 * 
	 * @return true
	 *            if satisfied, false if never in parked state or if queuing 
	 ...
	 * 
	 * @return true
	 *            if vehicle was or is in a parked state, false otherwise 
	 */
	 ...
	 * 
	 * @return true
	 *            if vehicle was or is in a queued state, false otherwise 
	 */
